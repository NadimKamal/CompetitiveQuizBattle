<?php
	...
	$key="";
	try{
        $get_data = "SELECT REQUEST_ID RID FROM [QB_DB].[dbo].[CM_BATTLE_REQUEST] WHERE SENDER_PARTY='$id' and REQUEST_DATE=(SELECT MAX(REQUEST_DATE) FROM [QB_DB].[dbo].[CM_BATTLE_REQUEST] WHERE SENDER_PARTY='$id')";
		$conn = Openconnection();
		...
		$_SESSION['temp_notification'] = $key;
?>
...
<body>
	...
	<div id="checkStatus"  style="display:;"></div>
	...
	var countDownDate = parseInt(tDate);
	...
	var x = setInterval(function() {
	  ...
	  var distance = (countDownDate + 120200) - now;
	  ...
	  let cStatus = String(document.getElementById("checkStatus").innerText);
	  //alert(cStatus);
	  if(cStatus=="R")
	  {
		clearInterval(x);
		document.getElementById("lazy").innerHTML = "Rejected";
	  ...
	  {
		//clearInterval(x);
		document.getElementById("lazy").innerHTML = "Accepted";
		window.location.assign("challenge-page.php");
	  }
	  // If the count down is over
	  else if (distance < 0) {
		clearInterval(x);
		window.location.assign("home.php?ChallangeReject=R");
